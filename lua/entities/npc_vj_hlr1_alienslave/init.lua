		...
		for i = 1, 4 do
			...
			timer.Simple(randTime, function()
				if IsValid(self) then
					...
					if tr.Hit then self:Vort_DoElecEffect(tr.StartPos, tr.HitPos, tr.HitNormal, 1, randTime) end
				end
			end)
		end
		-- Ach --------------------------
		...
		for i = 1, 4 do
			...
			timer.Simple(randTime, function()
				if IsValid(self) then
					...
					if tr.Hit then self:Vort_DoElecEffect(tr.StartPos, tr.HitPos, tr.HitNormal, 2, randTime) end
				end
			end)
		end
	end
...
function ENT:HandleGibOnDeath(dmginfo, hitgroup)
	...
	self:CreateGibEntity("obj_vj_gib", "models/vj_hlr/gibs/islavegib.mdl", {BloodType = "Yellow", CollisionDecal = "VJ_HLR1_Blood_Yellow", CollideSound = gibsCollideSd, Pos = self:LocalToWorld(Vector(1, 0, 40))})
