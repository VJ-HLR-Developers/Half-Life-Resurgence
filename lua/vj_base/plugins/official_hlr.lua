...
//VJ.AddNPC("Charger", "npc_vj_hlr1_charger", spawnCategory)
...
if CLIENT then
    ...
    net.Receive("VJ_HLR_FlashFX", function()
		...
		local hookName2 = "VJ_HLR_ScreenFlashFX" .. ply:EntIndex()
		hook.Add("HUDPaint", hookName2, function()
			curTime = CurTime()
			if !IsValid(ply) or (IsValid(ply) && (ply:Health() <= 0 or ply.VJ_HLR_FlashFXT < curTime)) then hook.Remove("HUDPaint", hookName2) return end
			if LocalPlayer() == ply then
				local alpha = curTime < startFade && color.a or math.Clamp(color.a - fadeSc * (curTime - startFade), 0, color.a)
				if alpha == 0 then
					hook.Remove("HUDPaint", hookName2)
					return
				end
